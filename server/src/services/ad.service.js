...
const getAdBySlug = async (slug) => {
  const { data: ad, error } = await supabase
    .from('ads')
    .select(`*, category:categories(*), city:cities(*), package:packages(*), media:ad_media(*), seller:users!user_id(full_name), payment:payments(status)`)
    .eq('slug', slug)
    .eq('status', 'published')
    .maybeSingle();
...
const getReviewQueue = async () => {
  const { data, error } = await supabase
    .from('ads')
    .select(`*, user:users!user_id(full_name,email), media:ad_media(*), category:categories(name), city:cities(name), package:packages(name)`)
...
const publishAd = async (admin_id, ad_id, { scheduled_at, feature } = {}) => {
  const { data: ad } = await supabase
    .from('ads')
    .select('*, package:packages(*), user:users!user_id(full_name, seller_profiles(is_verified))')
    .eq('id', ad_id)
    .maybeSingle();
