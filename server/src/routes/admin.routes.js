...
router.get('/analytics', ...isAdmin, async (req, res, next) => {
  try {
    ...
    const byCategory = (publishedAdsWithCategory || []).reduce((acc, row) => {
      const name = row?.categories?.name || 'Uncategorized';
      const existing = acc.find((item) => item.category === name);
      if (existing) {
        existing.count += 1;
      } else {
        acc.push({ category: name, count: 1 });
      }
      return acc;
    }, []);
